...
def get_reception_rate(events, positions):
    ...
    enhanced_sent_events = pl.concat([sent_events, positions_at_sent_time.with_columns(pl.lit('position').alias('eventType'))], how='diagonal').sort(['timestamp','eventType'], descending=True)
...
def main(v, event_log, position_log):
    ...
    loss_rate.to_csv(f'./res/v{v}/loss_rate_{event_log}.csv')
