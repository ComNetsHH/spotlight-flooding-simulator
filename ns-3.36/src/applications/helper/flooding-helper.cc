...
SLFFloodingAppHelper::SLFFloodingAppHelper(uint16_t port, Time sendInterval, Time forwardingJitter, uint32_t packetSize, double maxDistance, double decayFactor, double Threshold, uint32_t nodesPerContainer)
{
  m_factory.SetTypeId (SLFFloodingApp::GetTypeId ());
  SetAttribute ("Port", UintegerValue (port));
  SetAttribute ("SendInterval", TimeValue (sendInterval));
  SetAttribute ("ForwardingJitter", TimeValue (forwardingJitter));
  SetAttribute ("PacketSize", UintegerValue (packetSize));
  SetAttribute ("MaxDistance", DoubleValue (maxDistance));
  SetAttribute ("DecayFactor", DoubleValue (decayFactor));
  SetAttribute ("Threshold", DoubleValue (Threshold));
  SetAttribute ("nodesPerContainer", UintegerValue (nodesPerContainer));
  //SetAttribute("groundStationIds", VectorValue(groundStationIds));
...
void 
SLFFloodingAppHelper::SetAttribute (
  std::string name, 
  const AttributeValue &value)
{
  m_factory.Set (name, value);
...
ApplicationContainer
SLFFloodingAppHelper::Install (Ptr<Node> node) const
{
  return ApplicationContainer (InstallPriv (node));
}

ApplicationContainer
SLFFloodingAppHelper::Install (std::string nodeName) const
{
  Ptr<Node> node = Names::Find<Node> (nodeName);
  return ApplicationContainer (InstallPriv (node));
}
...
ApplicationContainer
SLFFloodingAppHelper::Install (NodeContainer c) const
{
  ApplicationContainer apps;
  for (NodeContainer::Iterator i = c.Begin (); i != c.End (); ++i)
  {
...
Ptr<Application>
SLFFloodingAppHelper::InstallPriv (Ptr<Node> node) const
{
  Ptr<Application> app = m_factory.Create<SLFFloodingApp> ();
  node->AddApplication (app);
  return app;
}
