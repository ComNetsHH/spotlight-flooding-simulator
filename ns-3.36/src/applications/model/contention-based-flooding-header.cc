...
namespace ns3 {
  ...
  // New methods for destination ID:
  uint32_t ContentionBasedFloodingHeader::GetDest() {
    return this->dest;
  }

  void ContentionBasedFloodingHeader::SetDest(uint32_t dest) {
    this->dest = dest;
  }
 ...
 void ContentionBasedFloodingHeader::SetControlSeq(uint32_t controlSeq){
    this->controlSeq = controlSeq;
  }

uint32_t ContentionBasedFloodingHeader::GetControlSeq(void) const{
    return controlSeq;
  }
  ...
  void ContentionBasedFloodingHeader::Print(std::ostream &os) const {
    ...
    os << "(startPos=" << start_pos << " seq=" << seq << " time="<< TimeStep(ts).As(Time::S) << " src=" << src << " lastHop=" << lastHop << " numHops=" << numHops << " dest=" << dest << " controlSeq=" << controlSeq <<  ")";
  }
