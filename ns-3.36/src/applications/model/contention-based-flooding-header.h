...
namespace ns3
{
    ...
    public:
        ...
        void SetControlSeq (uint32_t controlSeq);
        uint32_t GetControlSeq(void) const;
    ...
    private:
        ...
        uint32_t src = 0;
        uint32_t lastHop = 0;
        uint32_t dest = 0; // New member for destination ID
        uint32_t seq = 0;
        uint64_t ts = 0;
        Vector start_pos = Vector(0, 0, 0);
        Vector last_pos = Vector(0, 0, 0);
        uint32_t controlSeq = 0; // New member for control sequence number
    };
