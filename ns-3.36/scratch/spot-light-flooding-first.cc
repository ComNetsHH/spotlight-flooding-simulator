...
void OnPacketReceive(std::string context, Ptr<const Packet> pkt, uint32_t nodeId)
{
  ...
  string type = "PktRcvd";
     ...
     // Only add unique control sequence numbers for node 4
    if (nodeId == 4 && dest == 4){
        uniqueReceivedControlSeqNos.insert(controlSeqNo);
    }
  ...
  // uint32_t node_id, string eventType, string src, string dest, string lastHop, string delay, string numHops
 //}
...
void GetKPIs(NodeContainer c, int numNodes)
{
    ...
    NS_LOG_UNCOND(Simulator::Now().As(Time::S) << " P_D = " << pd << ", P_EX = " << pe500 << ", PDR = " << pdr);
}
...
int main(int argc, char *argv[])
{
    ...
    double speedMax = 33;
    bool tracing = true;
    uint32_t nodesPerContainer = 1; // Default value
    ...
    // Assuming 'c' is original NodeContainer that includes node 4 which is our fixed ground station. 
    

NodeContainer mobileNodes;
for (uint32_t i = 0; i < c.GetN(); ++i) {
    if (i != 4) { // Exclude node 4
   ...
   mobility.Install(mobileNodes);
    ...
    // Create a fixed position for node 4 
    
//'size' is the length of the sides of square area

std::vector<uint32_t> groundStationIds = {4};

MobilityHelper fixedMobility;
fixedMobility.SetMobilityModel("ns3::ConstantPositionMobilityModel");

// Create a NodeContainer for the fixed node (node 4)
NodeContainer fixedNode;
fixedNode.Add(c.Get(4)); // Assuming node 4 is the one to be fixed

fixedMobility.Install(fixedNode);

// Set the position of the fixed node to the center of the square
Ptr<ListPositionAllocator> positionAlloc = CreateObject<ListPositionAllocator>();
positionAlloc->Add(Vector(size / 2.0, size / 2.0, 0)); // Z-coordinate is 0 for 2D mobility
...
*/
    ...
    //client.SetDest(5); // Set the fixed destination node ID here
